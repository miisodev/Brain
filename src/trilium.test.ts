...
describe("backlinkRelationNames", () => {
  test("always includes the full canonical synapse vocabulary", () => {
    const names = backlinkRelationNames([]);
    for (const canonical of SynapseTypes) {
  ...
  test("excludes synaptic-weight labels and the internal template relation", () => {
    const names = backlinkRelationNames(["template", "sw_relatesTo_abc", "realRel"]);
    expect(names).not.toContain("template");
    expect(names.some((n) => n.startsWith("sw_"))).toBe(false);
